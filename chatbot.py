...
class Chatbot:
    ...
    def __init__(self, temperature=0):
        ...
        self.memory = []  # Initialize memory as a list to store context chunks
        self.context = ""
    ...
    def add_memory_from_chunks(self, chunks):
        context = "Answer user questions using info from provided research paper chunks (with IDs that are in the text). In your responses, reference the chunk IDs used. Format answers in JSON."
        self.memory.append({"role": "user", "content": chunks})
    ...
    def add_memory_fromchat(self,role,message_to_add):
        match role:
            case "user": 
                self.memory.append({"role":"user","content":message_to_add})
            case "assistant":
                self.memory.append({"role":"assistant","content":message_to_add})
    ...
    def reset_memory(self):
        """Reset the chatbot's memory, clearing all previous context."""
        self.memory.clear()
