...
from download_pdf import download_pdfs_parallel
from get_relevant_papers import get_relevant_papers
from typing import Dict
from langchain_anthropic import ChatAnthropic
from lit_review_agent_models import AgentState, PaperInformation, InsightGeneration
from prompts import THEME_IDENTIFICATION_PROMPT, PAPER_VALIDATION_PROMPT, INFORMATION_EXTRACTION_PROMPT, \
        ...
        for theme in (state.themes):
            ...
            for i in range(0, len(papers), papers_to_download):
                if not (papers_to_download - papers_found > 0):
                    break
                downloaded_papers = download_pdfs_parallel(paper.oa_url for paper in papers[i:i + papers_to_download])
                for paper in papers[i:i + papers_to_download]:
                    if downloaded_papers.get(paper.oa_url):
                        paper.pdf_content = downloaded_papers.get(paper.oa_url)
